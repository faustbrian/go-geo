// Package gogeom converts between go-geo and github.com/twpayne/go-geom.
...
// ToGoGeom converts an owned go-geo geometry through canonical EWKB. It is
...
// FromGoGeom converts a two-dimensional go-geom value with a positive SRID.
...
func FromGoGeom(value geom.T, limits geo.Limits) (geo.Geometry, error) {
	...
	if value.SRID() <= 0 {
		return nil, &geo.CRSError{
			...
			Problem: "go-geom conversion requires a positive SRID",
		}
	}
	limits = geo.ResolveLimits(limits)
	coordinates := value.FlatCoords()
	if len(coordinates)%2 != 0 {
		return nil, adapterError("go-geom has malformed XY coordinates", nil)
	}
	...
	if err != nil {
		return nil, adapterError("cannot encode go-geom EWKB", err)
	}
