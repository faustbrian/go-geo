...
`go-geo` makes coordinate order, CRS, numerical model, and resource limits
explicit. It provides immutable values and geometry, spherical and WGS84
geodesy, bounded GeoJSON/WKT/WKB codecs, geohashes, go-geom conversion, and
...
- `adapter/gogeom`: isolated conversion to and from `go-geom`.
...
- Empty geometry is not part of the v1 model. Degenerate, open,
  self-intersecting, and overlapping rings are rejected.
